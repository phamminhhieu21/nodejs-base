        ...
        const resp = await db.User.findOrCreate( // findOrCreate will return array [object, boolean]
            {
                where: {email}, defaults: {
                    email, password: hashPassword(password), name, phone_number, gender, date_of_birth
                }
            })
        const accessToken = resp[1] ? jwt.sign({
            id: resp[0].id,
            email: resp[0].email,
            role_code: resp[0].role_code,
            name: resp[0].name
        }, process.env.JWT_SECRET, {expiresIn: '1h'}) : null // if user is not exist, create access token
        const refreshToken = resp[1] ? jwt.sign({id: resp[0].id}, process.env.JWT_REFRESH_SECRET, {expiresIn: '7d'}) : null // if user is not exist, create refresh token
        resolve({
            code: resp[1] ? 0 : 1,
...
export const verifyRegisterMailService = (token) => new Promise(async (resolve, reject) => {
    try {
        ...
        if (resp) {
            ...
            resolve({
                code: 0, message: 'Register success, you can login now'
            })
        } else {
            resolve({
                code: 1, message: 'Email not exist'
            })
        }
    } catch (err) {
        reject(err)
...
export const login = (email, password) => new Promise(async (resolve, reject) => {
    try {
        const resp = await db.User.findOne({
            where: {email, is_active: true}, raw: true // return plain object instead of sequelize object
        })
        ...
        if (resp?.passwordReset) {
            ...
            if (isMatchPasswordReset) {
                await db.User.update({
                    password: resp.passwordReset, passwordReset: null,
                }, {
                    where: {id: resp.id}
                })
            }
        ...
        const accessToken = isCheckedPassword ? generateAccessToken({
            id: resp.id,
            email: resp.email,
        ...
        const refreshToken = isCheckedPassword ? jwt.sign({id: resp.id}, process.env.JWT_REFRESH_SECRET, {expiresIn: '7d'}) : null
    ...
    try {
        ...
        if (response) {
            jwt.verify(refresh_token, process.env.JWT_REFRESH_SECRET, (err) => {
                if (err) {
                    resolve({
                        code: 1, message: 'Refresh token expired. Require login'
                    })
                } else {
            ...
            resolve({
                code: 1, message: 'Refresh token not found'
            })
        }
    } catch (error) {
        reject(error)
...
export const verifyLoginProfile = (idGoogle, tokenLogin) => new Promise(async (resolve, reject) => {
    try {
        ...
        const response = await db.User.findOne({
            where: {idGoogle, tokenLogin}, raw: true
        })
...
export const changePassword = (id, newPassword, oldPassword) => new Promise(async (resolve, reject) => {
    try {
        const resp = await db.User.findOne({where: {id}})
        ...
        if (isMatchPassword) {
            ...
            resolve({
                code: 0, message: 'Change password success'
            })
        } else {
            resolve({
                code: 1, message: 'Wrong old password'
            })
        }
    } catch (err) {
        reject(err)
...
export const forgotPassword = (email) => new Promise(async (resolve, reject) => {
    try {
        const resp = await db.User.findOne({where: {email}})
        const token = resp && generateToken({id: resp.id, email: resp.email, role_code: resp.role_code}, '5m')
        const passwordTemp = resp && await generateTemporaryPassword(8)
        if (resp && token) {
            await db.User.update({
                passwordReset: passwordTemp, tokenResetPassword: token, isForgotPassword: true
            }, {
                where: {id: resp.id}
            })
            ...
            resolve({
                code: 0, message: 'Please check your email to reset password'
            })
        } else {
            resolve({
                code: 1, message: 'Email not exist'
    ...
    try {
        ...
        if (resp) {
            jwt.verify(token, process.env.JWT_SECRET, (err) => { // verify token is expired or not ?
                if (err) {
                    resolve({
                        code: 1, message: 'Token expired. Require reset password again'
                    })
                } else {
            ...
            resolve({
                code: 1, message: 'Token not found'
            })
        }
        if (checkToken) { // if token is not expired, reset password and set tokenResetPassword = null in db User
            await db.User.update({
                password: hashPassword(password), tokenResetPassword: null, isForgotPassword: false, passwordReset: null
            }, {
                where: {tokenResetPassword: token}
            })
            resolve({
                code: 0, message: 'Reset password success'
            })
        }
    } catch (e) {
        reject(e)
