...
export const verifyRegisterMailController = async (req, res) => {
  try {
    ...
    if (!token) return res.redirect(`${process.env.URL_CLIENT}/register/verify/failed`)
    const response = await services.verifyRegisterMailService(token)
    if (response.code !== 0) return res.redirect(`${process.env.URL_CLIENT}/register/verify/failed`)
    return res.redirect(`${process.env.URL_CLIENT}/register/verify/successful`)
  } catch (err) {
    return internalServerError(res)
  }
}
